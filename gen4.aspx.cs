...
public partial class gen4 : System.Web.UI.Page
{
    ...
    private void insertkhitoisl(string d, string dtime, string atime, int i)
        ...
        SqlCommand cmd = new SqlCommand("insert into ars_flights values ('" + fn + "','" + ddate + "','" + dtime + "','" + ddate + "','" + atime + "',(select price from ars_prices where distance = 'KHIISL'),'" + airport_code[i] + "','AR-KHI-001','" + plane_num[i] + "','Active')", con);
        con.Open();
        cmd.ExecuteNonQuery();
        con.Close();
    }
