...
public partial class view_schedule : System.Web.UI.Page
{
    ...
    public string getdata(string fn , string price)
    {
        ...
        con.Close();
        SqlCommand cmd1 = new SqlCommand("select * from ars_ticket where flight_num = '" + fn + "' and (status = 'Booking Confirmed' and status = 'Incomplete Booking')", con);
        con.Open();
        ...
        while (dr.Read())
        {
            ...
            if (dr["status"].ToString() == "Booking Confirmed")
            {
                clas = "success";
            }
